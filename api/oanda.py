...
class OandaApi:
    ...
    def get_accounts_ep(self,ep,data_key):
    ...
    def get_account_summary(self):
        return self.get_accounts_ep("summary","account")
    def get_instruments(self):
        return self.get_accounts_ep("instruments","instruments")
    ...
    def web_api_candles(self,pair_name,granularity,count):
        ...
        if df.shape[0] == 0:
            return None
        cols=['time','mid_c','mid_o','mid_h','mid_l']
        df=df[cols].copy()
    ...
    def place_trade(self,pair_name:str, units:float, direction: int, stop_loss:float=None,take_profit: float=None):
        ...
        data= dict(order=dict(units=str(units),
